...
PROJECT = ROOT / "projects" / "slide09_whitepaper_static_v7"
ASSET_DIR = PROJECT / "assets"
OUT_DIR = PROJECT / "exports"
VALIDATION_DIR = PROJECT / "validation"
OUT = OUT_DIR / "slide09_whitepaper_static_v7.pptx"
...
def add_factor_node(slide, cx, cy, data, name, label_pos="bottom"):
    ...
    if label_pos == "top":
        ...
        y0 = cy + 0.67
    textbox(slide, cx - 0.62, y0, 1.24, 0.16, data["cn"], 9.4, INK, True, PP_ALIGN.CENTER, f"{name}_cn")
    textbox(slide, cx - 0.72, y0 + 0.19, 1.44, 0.11, data["en"], 5.5, MUTED, False, PP_ALIGN.CENTER, f"{name}_en")
    textbox(slide, cx - 0.78, y0 + 0.34, 1.56, 0.12, data["level"], 5.0, MUTED, False, PP_ALIGN.CENTER, f"{name}_level")
    textbox(slide, cx - 0.78, y0 + 0.51, 1.56, 0.10, data["note"], 4.8, STEEL, False, PP_ALIGN.CENTER, f"{name}_note")

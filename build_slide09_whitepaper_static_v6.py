...
PROJECT = ROOT / "projects" / "slide09_whitepaper_static_v6"
ASSET_DIR = PROJECT / "assets"
OUT_DIR = PROJECT / "exports"
VALIDATION_DIR = PROJECT / "validation"
OUT = OUT_DIR / "slide09_whitepaper_static_v6.pptx"
...
def add_factor_node(slide, cx, cy, data, name):
    ...
    textbox(slide, cx - 0.62, cy + 0.68, 1.24, 0.16, data["cn"], 10.0, INK, True, PP_ALIGN.CENTER, f"{name}_cn")
    textbox(slide, cx - 0.72, cy + 0.88, 1.44, 0.11, data["en"], 5.9, MUTED, False, PP_ALIGN.CENTER, f"{name}_en")
    textbox(slide, cx - 0.78, cy + 1.05, 1.56, 0.18, data["level"], 5.4, MUTED, False, PP_ALIGN.CENTER, f"{name}_level")
    textbox(slide, cx - 0.86, cy + 1.27, 1.72, 0.20, data["note"], 5.25, STEEL, False, PP_ALIGN.CENTER, f"{name}_note")
...
def add_matrix(slide):
    ...
    for idx, ((nx, ny), data) in enumerate(zip(positions, FACTORS), start=1):
        add_factor_node(slide, nx, ny, data, f"factor_{idx:02d}")
    ...
    for i, label in enumerate(labels):
        ...
        textbox(slide, x + 0.08, footer_y + 0.00, 0.24, 0.10, f"{i+1:02d}", 5.4, CYAN, True)
        textbox(slide, x + 0.08, footer_y + 0.20, 1.10, 0.12, label, 6.8, INK, True)
...
def build():
    ...
    textbox(slide, 14.95, 8.48, 0.36, 0.13, "09", 8.2, WHITE, True, PP_ALIGN.CENTER)
    ...
    # Move footer page number text above the tab.

...
PROJECT = ROOT / "projects" / "slide09_whitepaper_static_v4"
ASSET_DIR = PROJECT / "assets"
OUT_DIR = PROJECT / "exports"
VALIDATION_DIR = PROJECT / "validation"
OUT = OUT_DIR / "slide09_whitepaper_static_v4.pptx"
...
def make_assets():
    ...
    # Local raster asset used as a substitute if built-in image generation output is not exposed as a file.
    scene = Image.new("RGB", (1500, 820), (245, 249, 250))
    d = ImageDraw.Draw(scene, "RGBA")
    for y in range(820):
        shade = int(252 - y * 0.014)
        d.line([(0, y), (1500, y)], fill=(shade, shade, shade, 255))
    d.rounded_rectangle((64, 55, 1435, 760), radius=36, outline=(198, 220, 226, 175), width=4)
    d.rounded_rectangle((155, 500, 1335, 590), radius=18, fill=(210, 233, 239, 240), outline=(130, 172, 184, 230), width=3)
    d.rectangle((220, 468, 1270, 505), fill=(239, 250, 252, 175))
    d.ellipse((540, 468, 960, 570), fill=(75, 172, 198, 24))
    d.rounded_rectangle((555, 250, 945, 468), radius=10, fill=(171, 184, 187, 255), outline=(95, 118, 124, 255), width=4)
    for i in range(27):
        x = 572 + i * 13
        d.line([(x, 265), (x + 104, 454)], fill=(238, 244, 245, 32), width=2)
    d.line((750, 98, 750, 250), fill=(0, 72, 84, 235), width=8)
    d.polygon([(728, 124), (772, 124), (750, 88)], fill=(0, 72, 84, 235))
    for x in [280, 430, 1030, 1185]:
        d.line((x, 545, x + 78, 500), fill=(255, 255, 255, 155), width=2)
    scene = scene.filter(ImageFilter.UnsharpMask(radius=1.1, percent=150, threshold=3))
    scene_path = ASSET_DIR / "metal_ice_contact_inset_v4.png"
